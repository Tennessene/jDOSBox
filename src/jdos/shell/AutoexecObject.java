...
public class AutoexecObject {
    private static Vector<String> autoexec_strings = new Vector<String>();
    private boolean installed = false;
    private String buf;

    public static void Shutdown() {
        autoexec_strings = new Vector<String>();
    }

    public void Install(String in) {
        if(installed) Log.exit("autoexec: allready created %s",buf);
        installed = true;
        buf = in;
    ...
    public void InstallBefore(String in) {
        if(installed) Log.exit("autoexec: already created %s",buf);
        installed = true;
        buf = in;
        autoexec_strings.add(buf);
        CreateAutoexec();
