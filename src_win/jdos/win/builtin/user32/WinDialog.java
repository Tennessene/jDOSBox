...
public class WinDialog extends WinAPI {
    ...
    static public int DIALOG_ParseTemplate32(int template, DLG_TEMPLATE result) {
        ...
        dlgver = readw(p); p+=2;
        signature = readw(p); p+=2;

        if (dlgver == 1 && signature == 0xffff) { /* DIALOGEX resource */
            result.dialogEx = true;
            result.helpId   = readd(p);p+=4;
            result.exStyle  = readd(p);p+=4;
            result.style    = readd(p);p+=4;
        }
        else
        {
        ...
        result.nbItems = readw(p);p+=2;
        result.x       = readw(p);p+=2;
        result.y       = readw(p);p+=2;
        result.cx      = readw(p);p+=2;
        result.cy      = readw(p);p+=2;
    ...
    static private int DIALOG_GetControl32(int p, DialogControlInfo info, boolean dialogEx) {
        if (dialogEx) {
            info.helpId  = readd(p);p+=4;
            info.exStyle = readd(p);p+=4;
            info.style   = readd(p);p+=4;
        } else {
            info.helpId  = 0;
            info.style   = readd(p);p+=4;
            info.exStyle = readd(p);p+=4;
