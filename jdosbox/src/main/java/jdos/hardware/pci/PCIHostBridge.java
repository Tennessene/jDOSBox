...
public class PCIHostBridge extends PCI_Device {
    public PCIHostBridge(){
        ...
        Memory.phys_writed(pci_routing, 0x52495024);		// signature
        ...
        pci_routing+=0x20;
        for (int i=0; i<6; i++) {
            Memory.phys_writeb(pci_routing, 0x00);				// bus number
            Memory.phys_writeb(pci_routing + 0x01, 0x08 + i * 8);			// device number
            pci_routing+=0x02;
            for (int link=0; link<4; link++) {
                Memory.phys_writeb(pci_routing, 0x60 + ((i + link) & 3));	// link value
                Memory.phys_writew(pci_routing + 0x01, 0xdef8);				// bitmap
                pci_routing+=0x03;
            }
            Memory.phys_writeb(pci_routing, i);				// slot
            Memory.phys_writeb(pci_routing + 0x01, 0x00);
            pci_routing+=0x02;
        }

        Memory.phys_writeb(pci_routing, 11);		// irq
